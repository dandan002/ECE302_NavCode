...
void HE_ISR_Stop(void)
{
    ...
    HE_ISR_SetVector(&IntDefaultHandler);
}
...
{
    #ifdef HE_ISR_INTERRUPT_INTERRUPT_CALLBACK
        HE_ISR_Interrupt_InterruptCallback();
    #endif /* HE_ISR_INTERRUPT_INTERRUPT_CALLBACK */ 
...
void HE_ISR_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)HE_ISR__INTC_NUMBER] = address;
}
...
cyisraddress HE_ISR_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)HE_ISR__INTC_NUMBER];
}
...
*
*   Note calling HE_ISR_Start or HE_ISR_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after HE_ISR_Start or HE_ISR_StartEx has been called. 
...
uint8 HE_ISR_GetState(void)
{
    ...
    return ((*HE_ISR_INTC_SET_EN & (uint32)HE_ISR__INTC_MASK) != 0u) ? 1u:0u;
}
